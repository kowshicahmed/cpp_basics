...
public:
    ...
    Person(std::string name, int age)
        : name{name}, age{age} {}
    bool operator<(const Person &rhs) const         // set uses this operator to sort
    {
...
void test1()
{
    ...
    s1 = {1, 2, 3 , 1, 1, 2, 2, 3, 3, 4, 5};        // items will be added only once
    display(s1);
...
void test3()
{
    ...
    auto result = s.insert("D");            // this returns std::pair with first and second
    display(s);
    
    std::cout << std::boolalpha;
    std::cout << "first: " << *(result.first) << std::endl;             // first is a pointer
